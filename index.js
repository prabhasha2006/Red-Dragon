...
async function startRedDragonMdNx() {
    ...
    // Get Profile Picture Group
       ...
       RedDragonMdNx.send5ButImg(pea[0].id, `「 تم تغيير إعدادات المجموعة 」\n\nتم تغيير اسم المجموعة الى *${pea[0].subject}*`, `رسالة تغيير إعدادات المجموعة`, wm_fatih, [])
     }
    })
...
⭐✑ مرحبا 👋 @${num.split("@")[0]},
⭐✑ يرحب بك ${metadata.subject}

⭐✑ وصف القروب: ${metadata.desc}

⭐✑ هاي انا بوت رد تلقائي وحمايه القروب مرحبا بك في القروب يرجى الالتزام بقوانين القروب او سوف يتم ازالتك تلقائيا 🚯🤖`} )
                } else if (anu.action == 'remove') {
                    RedDragonMdNx.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `⭐✑ @${num.split("@")[0]} غادر ${metadata.subject}

⭐✑ وداعا سعداء بتعرف عليك يرجى  الانضمام مره اخرى حتى لا تفوتكم المواضيع الجديده 🤖` })
                }
            }
        } catch (err) {
            console.log(err)
    ...
    RedDragonMdNx.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       if (options.asApplication) type = 'application'
